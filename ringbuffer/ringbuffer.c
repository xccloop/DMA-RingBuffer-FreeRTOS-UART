...
uint32_t rb_put(ringbuffer_t *rb, const uint8_t *data, uint32_t len)
{
    ...
    /* first segment: from head to buffer end */
    ...
    /* barrier: ensure data written before head update (compiler only) */
    rb->head = head + len;
    return len;
}
...
{
    if (!rb) return false;
    return rb_available(rb) >= rb->capacity;
}
